...
public class BookingServiceImplementation implements BookingService {
    ...
    // private final Counter rabbitSentCounter;
    private final Counter rabbitSentReservationCounter;
    private final Counter rabbitSentCancellationCounter;
    ...
    public BookingServiceImplementation(
        ...
        this.bookingCounter = meterRegistry.counter("booking_total");
        // this.rabbitSentCounter = meterRegistry.counter("rabbitmq_messages_sent_total");
        this.rabbitSentReservationCounter = meterRegistry.counter("rabbitmq_messages_sent_total", "type", "reservation");
        this.rabbitSentCancellationCounter = meterRegistry.counter("rabbitmq_messages_sent_total", "type", "cancellation");
        ...
        for (RoomFilling room:patchRoomsInfoReq.getRooms()) {
            ...
            while (current.isBefore(last)) {
                ...
                if (room.getStatus().equals("reserved")){
                    ...
                    bookingCounter.increment(); //  метрика бронирования
                    queueProducer.putStatistics(bookingStatisticsMessage);
                    rabbitSentReservationCounter.increment(); // метрика RabbitMQ при успешном бронировании
                } else {
                    ...
                    queueProducer.putStatistics(bookingStatisticsMessage);
                    rabbitSentCancellationCounter.increment(); // метрика RabbitMQ при отмене бронирования
                }
                // queueProducer.putStatistics(bookingStatisticsMessage);
                // rabbitSentCounter.increment(); //  метрика RabbitMQ
            }
        }
    }
    ...
    public void removeBooking(UUID hotelUid, UUID userUid)
        ...
        for (Reservation res: reservations) {
            ...
            bookingStatisticsMessage.setReserved(false);
            queueProducer.putStatistics(bookingStatisticsMessage);
            rabbitSentCancellationCounter.increment(); // RabbitMQ отмена бронирования
